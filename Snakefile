...
rule counts:
    input:
        expand(counts_dir + "{sample}.counts.tsv.gz", sample=sample_name) + expand(
            ...
            counts_dir + "{sample}.interval_list", sample=sample_name
        ),
...
rule ScramblePart2:
    ...
    shell:
        ...
        # Produce MEIs.txt
        Rscript --vanilla $scrambleDir/cluster_analysis/bin/SCRAMble.R --out-name $xDir/clusters --cluster-file $clusterFile --install-dir $scrambleDir/cluster_analysis/bin --mei-refs $meiRef --ref {params.ref}/ref --no-vcf --eval-meis --cores {threads}
